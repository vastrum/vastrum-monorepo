...
impl<E: ExecutionProvider<Ethereum>> EthereumEvm<E> {
    ...
    fn block_env(block: &Block<Transaction, Header>, fork_schedule: &ForkSchedule) -> BlockEnv {
        ...
        // Cap excess_blob_gas to prevent u128 overflow in revm's fake_exponential().
        // The blob gas price at 100M excess is already astronomically high, so this
        // doesn't affect real-world behavior.
        const MAX_EXCESS_BLOB_GAS: u64 = 100_000_000;
        let blob_excess_gas_and_price = block
            .header
            .excess_blob_gas()
            .map(|v| BlobExcessGasAndPrice::new(v.min(MAX_EXCESS_BLOB_GAS), blob_base_fee_update_fraction))
