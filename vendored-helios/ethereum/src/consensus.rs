...
impl<S: ConsensusSpec, R: ConsensusRpc<S>> Inner<S, R> {
    ...
    pub async fn get_updates(&self) -> Result<Vec<Update<S>>> {
        ...
        // Filter stale updates that some beacon APIs incorrectly include
        let store_period = calc_sync_period::<S>(self.store.finalized_header.beacon().slot);
        let before = updates.len();
        updates.retain(|u| calc_sync_period::<S>(*u.signature_slot()) >= store_period);
        if updates.len() < before {
            warn!(
                target: "helios::consensus",
                "filtered {} stale updates (store_period={})",
                before - updates.len(), store_period
            );
        }
