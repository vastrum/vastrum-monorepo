...
mod test {
    ...
    async fn setup() -> (EthereumClient, Vec<RootProvider>) {
        ...
        join_all(vec![helios_client.wait_synced()]).await;

        (helios_client, vec![helios_provider, provider])
    }
    ...
    async fn rpc_equivalence_tests() {
        ...
        let (_handle1, providers) = setup().await;
        let helios_rpc = &providers[0];
