                ...
                select! {
                    block = block_recv.recv() => {
                        match block {
                            ...
                            None => {
                                warn!(target: "helios::client", "block channel closed, stopping node");
                                break;
                            }
                        }
                    },
                    finalized_changed = finalized_block_recv.changed() => {
                        if finalized_changed.is_err() {
                            warn!(target: "helios::client", "finalized block channel closed, stopping node");
                            break;
                        }
    ...
    for Node<N, C, E>
{
    ...
    async fn wait_head_synced(&self) -> Result<()> {
        // First wait for consensus sync
        self.wait_synced().await?;

        // Then wait for execution layer to have a recent block
        loop {
            match self.check_head_age().await {
                Ok(()) => return Ok(()),
                Err(ClientError::OutOfSync(delay)) => {
                    info!(
                        target: "helios::client",
                        "waiting for execution layer sync, {}s behind",
                        delay
                    );
                    #[cfg(not(target_arch = "wasm32"))]
    ...
    async fn get_blob_base_fee(&self) -> Result<U256> {
        ...
        if let Some(excess_blob_gas) = block.header().excess_blob_gas() {
            ...
            // Cap to prevent u128 overflow in revm's fake_exponential()
            let price = BlobExcessGasAndPrice::new(excess_blob_gas.min(100_000_000), blob_base_fee_update_fraction)
                .blob_gasprice;
            Ok(U256::from(price))
        } else {
            Ok(U256::ZERO)
