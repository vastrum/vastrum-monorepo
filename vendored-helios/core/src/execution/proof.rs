...
pub fn create_receipt_proof<N: NetworkSpec>(
    ...
    for i in 0..receipts_len {
        ...
        hb.add_leaf(Nibbles::unpack(&index_buffer), N::encode_receipt(&receipts[index]).as_slice());
    }
    ...
    hb.take_proof_nodes().into_nodes_sorted().into_iter().map(|n| n.1).collect::<Vec<_>>()
}
