...
The blockchain indexer on vastrum-node lives in [vastrum-node/src/block_indexer/indexer.rs (gitter preview)](https://yts27rvo7ppzq5rrjyavmfwecrbyc5ksldmitiggycetgh6zguoa.vastrum.net/repo/vastrum/tree/vastrum-node/src/block_indexer/indexer.rs). 

Everytime a block is executed the indexer is called which writes metadata for every transaction executed in the block.
...
Blocker is mostly just a prototype (Seemed cool to have a onchain chain explorer, a kind of ouroboros like challenge), final version of Vastrum will probably not support blocker.
