...
mod tests {
    ...
    async fn test_push_divergent_rejected() {
        ...
        let merge_res =
            merge_repos("test_repo", "feature-branch-1", &ctx.contract, MergeMode::Live)
                .await
                .unwrap();
        ...
        let merge_res =
            merge_repos("test_repo", "feature-branch-2", &ctx.contract, MergeMode::Live)
                .await
                .unwrap();
        ...
        let merge_res =
            merge_repos("test_repo", "feature-branch-3", &ctx.contract, MergeMode::Live)
                .await
                .unwrap();
