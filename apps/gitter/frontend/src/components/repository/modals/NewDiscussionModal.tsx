...
import { create_discussion } from '../../../../wasm/pkg';
...
function NewDiscussionModal({ isOpen, onClose, repoId, onRefresh }: NewDiscussionModalProps): React.JSX.Element {
    const [title, setTitle] = useState('');
    const [description, setDescription] = useState('');

    const handleSubmit = async (): Promise<void> => {
        const txHash = await create_discussion(title, description, repoId);
        onClose();
        setTitle('');
        setDescription('');
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="New Discussion">
            {/* Modal Body */}
